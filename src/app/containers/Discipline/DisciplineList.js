...
class DisciplineList extends Component {
    ...
    checkPermission() {
        canUser('discipline.update', this.props.history, "change", function(rules){
            if (rules.length == 0) {
    ...
    checkDeletePermission() {
        canUser('discipline.delete', this.props.history, "change", function(rules){
            if (rules.length == 0) {
    ...
    componentWillMount() {
        ...
        this.setState({ table_columns, initial_columns : table_columns }, function() {
            const table_preference = verifyPreferences(this.state.table_columns, 'prefs_discipline');
